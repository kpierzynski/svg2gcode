...
static Point quadratic_curve(Point p0, Point p1, Point p2, float t)
{
	Point p;
...
void svg_quadratic(uint8_t is_relative, char *args, Point *initial_point, Point *current_point)
{
	...
	while ((next = parse_ref_cub(next, &p1, &p2)))
	{
		...
		{
			Point result = quadratic_curve(p0, p1, p2, t);
			if (is_relative)
		...
		if (is_relative)
		{
			second_control = point_add(*current_point, p1);
			*current_point = point_add(*current_point, p2);
		}
		else
		{
			second_control = p1;
...
void svg_quadratic_s(uint8_t is_relative, char *args, Point *initial_point, Point *current_point)
{
	...
	float delta = 0.1;
	while ((next = parse_point(next, &p2)))
	{
		p0 = (is_relative) ? (Point){0, 0} : *current_point;
